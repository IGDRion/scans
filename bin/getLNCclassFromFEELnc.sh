...
do
    echo $pathToGTF ;
    grep -v "#" "$pathToGTF" | perl -ne "print if /$regEx_lncRNA/" > "${OUTDIR}/${completeName}_LNCextracted.tmp.gtf";
    grep -v "#" $pathToGTF | perl -ne "print if /$regEx_mRNA/" > "${OUTDIR}/${completeName}_mRNAextracted.tmp.gtf";
done
...
echo "getGeneLevelClassification.R "${CONFIG}" "${OUTDIR}""