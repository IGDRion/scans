...
# Header ---------------------------------------------------------------------------------------------
## Aim : get gene level classification from FEELnc for synteny method 2
...
## STEP1: preparation of input files
cat("STEP1: Input files preparation", "\n")

name.list <- config$completeName

for(i in 1:nrow(config)){
  sp_name <- config$completeName[i]
  short_name <- config$shortName[i]
  gtf.path <- config$pathToGTF[i]

  cat("- ", sp_name, " :\n")
  
  gtf <- as.data.frame(rtracklayer::import(gtf.path))

  # check if gene_name is present
  if (!"gene_name" %in% colnames(gtf)) {
    gtf$gene_name <- gtf$gene_id
    print("No 'gene_name' ; column created with 'gene_id'")
  } else {
    print("'gene_name' info available.")
  }
  ...
  #
  #########################
  cat("STEP2: Process FEELnc results at transcript level", "\n")
  ##-----------------------------------------------------##
  ...
  #
  #########################
  cat("STEP3: Process FEELnc results at gene level", "\n")
  ##-----------------------------------------------------##
